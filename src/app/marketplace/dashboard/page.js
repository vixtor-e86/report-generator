...
import ProposalDetailModal from '@/components/marketplace/tools/ProposalDetailModal';

export default function MarketplaceDashboardPage() {
  const { user } = useUser();
  const { wallet, refreshWallet, setShowFundingModal, deductFunds } = useWallet();
  ...
  }, [activeTab, user, fetchSellerData, fetchProposals]);
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fc] pb-20 font-sans">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-10">
        ...
        <div className="sticky top-[70px] z-30 bg-[#f8f9fc]/95 backdrop-blur-md pt-2 pb-6 mb-4">
          <div className="bg-zinc-100 border border-zinc-200 p-1 rounded-xl shadow-sm inline-flex">
            ...
            <button 
              ...
              className={`px-8 py-2.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ml-1 ${activeTab === 'proposals' ? 'bg-white shadow-sm text-black' : 'text-zinc-500'}`}
            >
              My Proposals
            </button>
            <button 
        ...
        {activeTab === 'proposals' && (
          <div className="space-y-8 animate-in fade-in duration-500">
             <div className="bg-white border border-[#e5e7eb] rounded-[40px] shadow-sm p-10">
                <div className="flex items-center gap-4 mb-10">
                  ...
                  <h2 className="text-xl font-black text-slate-900 uppercase tracking-tighter">My Technical Proposals</h2>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {proposals.map(prop => (
                        <div key={prop.id} className="flex items-center justify-between p-6 bg-zinc-50 rounded-[32px] border border-zinc-100 hover:border-indigo-400 transition-all group">
                            <div className="flex items-center gap-5 min-w-0">
                                <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-2xl border border-zinc-100 shadow-sm shrink-0">
                                    {prop.topic_data?.emoji || '📘'}
                                </div>
                                <div className="min-w-0">
                                    <h4 className="font-black text-slate-900 text-sm truncate uppercase tracking-tight">{prop.topic_title}</h4>
                                    <div className="flex items-center gap-3 mt-1">
                                        <Badge className={`px-2 py-0 text-[8px] font-black uppercase ${
                                            prop.status === 'generated' ? 'bg-emerald-100 text-emerald-700' : 
                                            prop.status === 'paid' ? 'bg-blue-100 text-blue-700' : 'bg-slate-100 text-slate-500'
                                        }`}>
                                            {prop.status}
                                        </Badge>
                            ...
                            <button 
                                ...
                                className="p-3 bg-white text-zinc-900 rounded-xl hover:bg-black hover:text-white transition-all shadow-sm border border-zinc-200"
                            >
                    ...
                    {proposals.length === 0 && !loadingProposals && (
                        <div className="md:col-span-2 py-20 text-center space-y-4 bg-slate-50/50 rounded-[40px] border-2 border-dashed border-slate-100">
                            ...
                            <div>
                                ...
                                <Link href="/marketplace/tools/project-finder">
                                    <button className="mt-4 text-[10px] font-black text-indigo-600 uppercase tracking-[0.2em] hover:underline">Launch Topic Finder →</button>
                                </Link>
                            </div>
                        </div>
                    )}
                    ...
                    {loadingProposals && (
                        <div className="md:col-span-2 py-20 text-center flex flex-col items-center gap-3">
                            ...
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Fetching Library...</p>
                        </div>
                    )}
                </div>
             </div>
      ...
      <ProposalDetailModal 
        ...
        onClose={() => { setSelectedProposal(null); fetchProposals(); }}
        topic={selectedProposal?.topic_data}
        userId={user?.id}
        walletBalance={wallet?.balance || 0}
        onDeductFunds={deductFunds}
        setShowFundingModal={setShowFundingModal}
        hasPaid={selectedProposal?.status === 'paid' || selectedProposal?.status === 'generated'}
        setHasPaid={() => {}} 
        setCustomPrice={() => {}}
