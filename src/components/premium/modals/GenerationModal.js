...
export default function GenerationModal({ 
  ...
  formData, setFormData // Using sticky props from parent
}) {
  ...
  // Initialize form data from projectData only if it's currently empty
  useEffect(() => {
    ...
    <>
      ...
      <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-3xl shadow-2xl w-full max-w-3xl overflow-hidden flex flex-col z-[1000] max-h-[90vh]">
        <div className="p-6 border-b flex justify-between items-center">
          <div>
            <h2 className="text-xl font-black text-slate-900">AI Architect: Chapter {currentChapterNumber}</h2>
            <p className="text-xs text-slate-500 font-bold uppercase tracking-widest">{activeChapter?.title}</p>
          </div>
          <button onClick={onClose} className="p-2 hover:bg-slate-100 rounded-full transition-colors"><Icons.X /></button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8">
          {activeTab === 'details' ? (
            <div className="space-y-6">
              ...
              <div>
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Instructions for AI</label>
                <textarea value={formData.userPrompt} onChange={(e) => setFormData({...formData, userPrompt: e.target.value})} placeholder="e.g. Focus on the sustainability aspect..." className="w-full h-32 p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-slate-900 outline-none transition-all font-medium text-slate-700 resize-none" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Ref Style</label>
            ...
            <div className="space-y-10">
              ...
              <div className="space-y-4">
                <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Select Visuals</h4>
        ...
        <div className="p-6 bg-slate-50 border-t flex justify-between items-center">
          <p className="text-[10px] font-black text-slate-400 uppercase">Configured: {formData.referenceStyle} | {formData.targetWordCount} Words</p>
        ...
        <AnimatePresence>
          {previewFile && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-[1100] bg-white p-8 flex flex-col">
              <div className="flex justify-between items-center mb-8">
                <h3 className="text-xl font-black text-slate-900 uppercase">Confirm Data Extraction</h3>
                <button onClick={() => setPreviewFile(null)} className="p-2 bg-slate-100 rounded-full"><Icons.X /></button>
              </div>
              <div className="flex-1 bg-slate-50 rounded-[32px] p-8 overflow-y-auto border-2 border-slate-100 mb-8">
                {previewLoading ? (
                  <div className="h-full flex items-center justify-center animate-pulse font-black text-slate-300">EXTRACTING TECHNICAL DATA...</div>
                ) : (
