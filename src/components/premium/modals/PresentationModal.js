        ...
        if (slide.imageDbId && bufferedImages[slide.imageDbId]) {
          ...
          if (slide.bullets) {
            slide.bullets.slice(0, 4).forEach((bullet, idx) => {
              const y = 25 + (idx * 14);
              pptSlide.addText('• ' + bullet, { x: '8%', y: `${y}%`, w: '40%', h: '12%', fontSize: 14, color: '334155' });
            });
          }
      ...
      return (
        <div style={{ ...commonStyles, backgroundColor: 'white', padding: '8%' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: slide.imageDbId ? '1fr 1fr' : '1fr', gap: '2rem' }}>
            ...
            {slide.imageDbId && (
              <div style={{ background: '#f8fafc', borderRadius: '16px', display: 'flex', itemsCenter: 'center', justifyContent: 'center', border: '2px dashed #e2e8f0', height: '200px' }}>
...
export default function PresentationModal({ isOpen, onClose, chapters, projectId, userId, setIsGlobalLoading, setGlobalLoadingText, showNotification, images = [] }) {
  ...
  const handleDownload = () => {
    ...
    const filename = `${slides[0].title.replace(/\s+/g, '_')}_Academic_Presentation.pptx`;
        ...
        <div className="flex-1 flex overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-6 md:p-12 flex flex-col items-center">
              {step === 'selection' && (
                <div className="w-full max-w-xl space-y-8 my-auto">
                  <div className="text-center">
                    <h3 className="text-3xl font-black text-slate-900 tracking-tight">Select Basis</h3>
                    <p className="text-slate-500 font-medium mt-2">Choose the chapters to be transformed into detailed technical slides.</p>
                  </div>
                    ...
                    {chapters.map(ch => {
                      ...
                      return (
                        <button key={ch.id} disabled={!hasContent} onClick={() => toggleChapter(ch.number)}
                          className={`flex items-center justify-between p-6 rounded-[32px] border-2 transition-all text-left ${isSelected ? 'border-slate-900 bg-white shadow-2xl ring-8 ring-slate-100' : 'border-slate-100 bg-white hover:border-slate-200'}`}>
                          <div className="flex-1 pr-4">
                            <span className={`text-[10px] font-black uppercase tracking-widest ${hasContent ? 'text-indigo-600' : 'text-slate-400'}`}>Chapter {ch.number}</span>
                            <h4 className={`text-base font-black truncate mt-1 ${hasContent ? 'text-slate-900' : 'text-slate-300'}`}>{ch.title}</h4>
                          </div>
                          {isSelected && <div className="w-8 h-8 bg-slate-900 rounded-full flex items-center justify-center text-white shadow-xl"><Icons.Check /></div>}
                        </button>
                      );
                    })}
                  </div>
                  <button onClick={() => setStep('images')} disabled={selectedChapters.length === 0}
                    className="w-full py-6 bg-slate-900 text-white rounded-[24px] font-black text-sm shadow-2xl transition-all active:scale-95 flex items-center justify-center gap-3">
                    NEXT: TECHNICAL VISUALS <Icons.ChevronRight />
                  </button>
                </div>
              )}
              ...
              {step === 'images' && (
                <div className="w-full max-w-3xl space-y-8 my-auto">
                  <div className="text-center">
                    <h3 className="text-3xl font-black text-slate-900 tracking-tight">Technical Visuals</h3>
                    <p className="text-slate-500 font-medium mt-2">Select diagrams or figures to embed directly into your presentation.</p>
                  </div>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-h-[45vh] overflow-y-auto p-4 custom-scrollbar">
                    {images.map(img => (
                      <button key={img.id} onClick={() => setSelectedImageIds(prev => prev.includes(img.id) ? prev.filter(i => i !== img.id) : [...prev, img.id])}
                        className={`relative aspect-square rounded-[24px] overflow-hidden border-4 transition-all ${selectedImageIds.includes(img.id) ? 'border-slate-900 scale-95 shadow-2xl' : 'border-white shadow-lg opacity-60 hover:opacity-100'}`}>
                  ...
                  <div className="flex gap-4">
                    <button onClick={() => setStep('selection')} className="flex-1 py-6 bg-white border-2 border-slate-100 rounded-[24px] font-black text-slate-400">BACK</button>
                    <button onClick={handleGenerate} className="flex-[2] py-6 bg-slate-900 text-white rounded-[24px] font-black text-sm shadow-2xl flex items-center justify-center gap-3">
                      <Icons.Sparkles /> BUILD DETAILED PREVIEW
                    </button>
                  </div>
                </div>
              )}
              ...
              {step === 'preview' && (
                <div className="w-full h-full flex flex-col items-center justify-center">
                  <div className="w-full max-w-[850px] aspect-video rounded-[40px] shadow-2xl overflow-hidden relative group border-[12px] border-white bg-white">
                    <SlideRenderer slide={slides[currentIndex]} template={selectedTemplate} />
                    <button onClick={() => setCurrentIndex(p => Math.max(0, p-1))} className="absolute left-6 top-1/2 -translate-y-1/2 p-4 bg-white/20 hover:bg-white/40 backdrop-blur-xl rounded-full text-white opacity-0 group-hover:opacity-100 transition-all"><Icons.ChevronLeft /></button>
                    <button onClick={() => setCurrentIndex(p => Math.min(slides.length-1, p+1))} className="absolute right-6 top-1/2 -translate-y-1/2 p-4 bg-white/20 hover:bg-white/40 backdrop-blur-xl rounded-full text-white opacity-0 group-hover:opacity-100 transition-all"><Icons.ChevronRight /></button>
                  </div>
                  <div className="mt-12 flex flex-col items-center gap-8">
                    <div className="flex gap-3">
                      {slides.map((_, i) => (
                        <button key={i} onClick={() => setCurrentIndex(i)} className={`h-2 rounded-full transition-all ${i === currentIndex ? 'w-16 bg-slate-900' : 'w-2 bg-slate-200'}`} />
                      ))}
                    </div>
                    <button onClick={() => setStep('images')} className="text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-slate-900">← Back to Customization</button>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="w-96 bg-white border-l border-slate-100 flex flex-col shrink-0">
            <div className="flex-1 overflow-y-auto p-10">
              <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-8">Presentation Theme</h4>
              <div className="space-y-4">
                {SLIDE_TEMPLATES.map(t => (
                  <button key={t.id} onClick={() => setSelectedTemplate(t)}
                    className={`w-full relative p-6 rounded-[28px] border-2 transition-all text-left overflow-hidden ${selectedTemplate.id === t.id ? 'border-slate-900 shadow-2xl ring-8 ring-slate-50' : 'border-slate-50 hover:border-slate-100'}`}>
                    <div className="absolute inset-0 opacity-10" style={{ backgroundColor: t.primaryColor }} />
                    <div className="relative z-10">
                      <div className="font-black text-slate-900 text-sm uppercase">{t.name}</div>
                      <div className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-tighter">{t.theme} Style</div>
                    </div>
                  </button>
                ))}
              </div>
            ...
            <div className="p-10 bg-slate-50 border-t border-slate-100">
              <button onClick={handleDownload} disabled={step !== 'preview'}
                className="w-full py-6 bg-slate-900 text-white rounded-[24px] font-black text-sm shadow-2xl flex items-center justify-center gap-3 disabled:opacity-40">
