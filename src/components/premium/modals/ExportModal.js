...
function SortableItem({ id, file }) {
  ...
  return (
    <div 
      ref={setNodeRef} 
      style={style} 
      className="flex items-center gap-3 p-4 bg-white border border-slate-100 rounded-2xl shadow-sm mb-2 group active:scale-[0.98] transition-all"
    >
      <div {...attributes} {...listeners} className="cursor-grab active:cursor-grabbing text-slate-300 group-hover:text-slate-500">
        <Icons.Grip />
      </div>
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Document Attachment</p>
      </div>
    </div>
  );
}

export default function ExportModal({ isOpen, onClose, type, projectDocs, chapters, projectId, userId, setIsGlobalLoading, setGlobalLoadingText }) {
  const [orderedDocs, setOrderedDocs] = useState([]);
  const [options, setFormData] = useState({
    ...
    try {
      const response = await fetch('/api/premium/export', {
        ...
        body: JSON.stringify({
          ...
          userId,
          type,
          orderedDocIds: orderedDocs.map(d => d.id),
          options
        })
      });
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-0 md:p-4">
      ...
      <motion.div initial={{ scale: 0.98, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} 
        className="relative bg-white md:rounded-[40px] shadow-2xl w-full max-w-5xl overflow-hidden flex flex-col h-full md:h-[90vh] md:max-h-[800px]">
        ...
        <div className="p-6 md:p-8 flex justify-between items-center bg-white border-b border-slate-100 shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Setup project structure and formatting</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-hidden flex flex-col md:flex-row">
          ...
          <div className="flex-1 flex flex-col bg-slate-50 border-r border-slate-100 overflow-hidden">
            <div className="p-6 border-b border-slate-100 bg-white/50 shrink-0">
              <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">Step 1: Order Attachments</h3>
              <p className="text-xs text-slate-500 font-medium mt-1">Drag files to arrange them before the project chapters (e.g. Cover Page, Acknowledgement).</p>
            </div>
              ...
              {orderedDocs.length > 0 ? (
                <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                  <SortableContext items={orderedDocs.map(d => d.id)} strategy={verticalListSortingStrategy}>
                    {orderedDocs.map(doc => (
                      <SortableItem key={doc.id} id={doc.id} file={doc} />
                    ))}
                  </SortableContext>
                </DndContext>
              ) : (
                <div className="h-full flex flex-col items-center justify-center text-center opacity-40">
                  ...
                  <p className="text-xs max-w-[200px] mt-1">Upload files like Cover Pages or certifications in the sidebar to order them here.</p>
                </div>
              )}
            </div>
          </div>

          {/* Options & Settings (Right) */}
            ...
            <div className="space-y-10">
              ...
              <section>
                ...
                <div className="space-y-4">
                  {/* Abstract Toggle */}
                  <ToggleItem 
                    active={options.includeAbstract} 
                    onClick={() => setFormData(p => ({ ...p, includeAbstract: !p.includeAbstract }))}
                    title="Generate Abstract"
                    desc="Claude will write a professional summary based on your chapters."
                  />
                  
                  {/* TOC Toggle */}
                  <ToggleItem 
                    active={options.includeTOC} 
                    onClick={() => setFormData(p => ({ ...p, includeTOC: !p.includeTOC }))}
                    title="Table of Contents"
                    desc="Automatically generate a linked TOC with page numbers."
                  />

                  {/* Page Numbers Toggle */}
                  <ToggleItem 
                    active={options.includePageNumbers} 
                    onClick={() => setFormData(p => ({ ...p, includePageNumbers: !p.includePageNumbers }))}
                    title="Page Numbering"
                    desc="Add standard academic numbering to footer (Page X of Y)."
                  />
                </div>
              </section>
              ...
              <section className="bg-slate-50 rounded-3xl p-6 border-2 border-dashed border-slate-200">
                {exportState === 'idle' && (
                  <div className="text-center">
                    <p className="text-xs font-bold text-slate-500 leading-relaxed mb-6">Review your order and settings. Your project images will be included automatically.</p>
                ...
                {exportState === 'processing' && (
                  <div className="text-center py-4">
                    ...
                    <p className="text-sm font-black text-slate-900 uppercase tracking-widest animate-pulse">Building Document...</p>
                  </div>
                )}

                {exportState === 'ready' && (
                  <div className="space-y-3">
                    <div className="flex items-center justify-center gap-2 text-emerald-600 font-black text-[10px] uppercase tracking-widest mb-4">
                      <Icons.Check /> Your file is ready
                    </div>
                    <a href={exportUrl} target="_blank" className="w-full py-4 bg-slate-900 hover:bg-black text-white rounded-2xl font-black text-xs flex items-center justify-center gap-2 no-underline transition-all">
                      <Icons.Eye /> PREVIEW
                    </a>
...
function ToggleItem({ active, onClick, title, desc }) {
  return (
    <button onClick={onClick} className={`w-full p-5 rounded-3xl border-2 transition-all text-left flex items-start gap-4 ${active ? 'border-slate-900 bg-white shadow-lg' : 'border-slate-50 bg-slate-50/50 opacity-60'}`}>
      <div className={`mt-1 w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all ${active ? 'bg-slate-900 border-slate-900 text-white' : 'bg-white border-slate-200'}`}>
        {active && <Icons.Check />}
      </div>
      <div>
        <p className="text-sm font-black text-slate-900 leading-none">{title}</p>
        <p className="text-[10px] text-slate-400 font-bold leading-relaxed mt-2 uppercase tracking-wide">{desc}</p>
      </div>
    </button>
  );
}
