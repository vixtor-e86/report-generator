...
export default function HumanizerModal({ isOpen, onClose, chapters, projectId, userId, projectData, setIsGlobalLoading, setGlobalLoadingText, onSaved, showNotification, onUpdateProjectData, humanizerLimit }) {
  ...
  const [selectedSectionId, setSelectedSectionId] = useState('all'); // SINGLE SELECT
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      showNotification('Success', 'Rewrite saved. Select another section.', 'success');
        ...
        <div className="p-5 md:p-8 flex justify-between items-center bg-white border-b border-slate-100 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Premium Research Suite</p>
            </div>
          </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-6 md:p-10 bg-slate-50 overflow-hidden">
              <div className="w-full max-w-md flex flex-col h-full max-h-[600px] space-y-6">
                ...
                <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm shrink-0">
                  <div className="flex justify-between items-end mb-2">
                    <div>
                      ...
                      <p className="text-sm font-black text-slate-900">{localUsage.toLocaleString()} / {limit.toLocaleString()} words</p>
                    </div>
                ...
                <div className="text-center shrink-0">
                  ...
                  <p className="text-sm text-slate-500 mt-1">Select a chapter with content to optimize.</p>
                </div>
                    ...
                    {chapters.map(ch => {
                      ...
                      return (
                        <button key={ch.id} disabled={!hasContent} onClick={() => setSelectedChapterId(ch.id)}
                          ...
                          {!hasContent && <span className="text-[8px] font-black bg-slate-100 px-2 py-1 rounded text-slate-400">EMPTY</span>}
                          {hasContent && <Icons.ChevronRight />}
                        </button>
                      );
                    })}
                  </div>
                ...
                <button onClick={() => setStep('sections')} disabled={!selectedChapterId}
                  className="w-full py-4 md:py-5 bg-slate-900 hover:bg-black text-white rounded-2xl md:rounded-3xl font-black text-sm shadow-2xl transition-all active:scale-95">
                  REFINE SELECTION
                </button>
              </div>
            </div>
              ...
              <div className="w-full max-w-md flex flex-col h-full max-h-[600px] space-y-6">
                ...
                <div className="flex gap-3 shrink-0">
                  ...
                  <button onClick={handleHumanize} disabled={isProcessing}
                    ...
                    <Icons.Sparkles /> START AI REWRITE
                  </button>
                </div>
              </div>
            </div>
          ...
          {step === 'processing' && (
            <div className="flex-1 flex flex-col items-center justify-center p-10 bg-slate-50">
              <div className="w-full max-w-md text-center space-y-8">
                ...
                <p className="text-sm text-slate-500 mt-2">Integrating academic language and structural flow.</p>
              </div>
            </div>
          )}
