          ...
          <div className="tools-list">
            <div className="tool-section">
              <h4 className="tool-group-title" style={{ color: '#059669', display: 'flex', alignItems: 'center', gap: '6px' }}>
                <span style={{ width: '8px', height: '8px', background: '#059669', borderRadius: '50%' }}></span> Available Tools
              </h4>
              <ToolItem icon={<Icons.Image style={{ color: '#6366f1' }} />} title="Diagram & Image Studio" desc="Create diagrams and illustrations." onClick={onVisualToolsClick} action="Use" />
              <ToolItem icon={<Icons.Search />} title="Semantic Scholar" desc="Find relevant papers and metadata." onClick={onSearchClick} action="Use" />
              <ToolItem icon={<Icons.User style={{ color: '#4f46e5' }} />} title="Humanizer Tool" desc="Bypass detectors and improve flow." onClick={onHumanizerClick} action="Use" />
              <ToolItem icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="3" width="20" height="14" rx="2" ry="2"></rect><line x1="8" y1="21" x2="16" y2="21"></line><line x1="12" y1="17" x2="12" y2="21"></line></svg>} title="Presentation Builder" desc="Generate PowerPoint slides." onClick={onPresentationClick} action="Use" />
            </div>
          ...
          <div className="files-panel p-4">
            <div className="mb-4">
              <label className={`flex items-center justify-center w-full p-4 border-2 border-dashed border-gray-300 rounded-lg hover:bg-gray-50 cursor-pointer transition ${uploading ? 'opacity-50 pointer-events-none' : ''}`}>
                <input type="file" className="hidden" onChange={handleQuickUpload} disabled={uploading} accept=".pdf,.docx,.txt,.xlsx" />
                <div className="text-center">
                  <div className="text-indigo-600 mb-1">{uploading ? 'Uploading...' : <Icons.Upload />}</div>
                  <span className="text-xs font-medium text-gray-600">{uploading ? 'Please wait' : 'Click to Upload Document'}</span>
                </div>
              </label>
            </div>
            <div className="files-list space-y-3">
              {files.length > 0 ? files.map((file) => (
                <div key={file.id} className="file-item flex items-start gap-3 p-3 bg-white border border-gray-100 rounded-lg hover:shadow-sm transition cursor-pointer" onClick={() => onFileClick(file)}>
                  <span className="text-xl">{file.file_type?.includes('image') ? '🖼️' : '📄'}</span>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-gray-900 truncate" title={file.original_name}>{file.original_name}</p>
                    <div className="flex justify-between items-center mt-1">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button onClick={(e) => { e.stopPropagation(); onDelete(file); }} disabled={deleting} className="text-red-500 hover:text-red-700 disabled:opacity-50"><Icons.Trash /></button>
                      </div>
                    </div>
                  </div>
                </div>
              )) : <div className="tool-placeholder text-center py-8"><span className="placeholder-icon text-4xl block mb-2">📂</span><p className="text-sm text-gray-500">No files uploaded yet.</p></div>}
            </div>
          </div>
        )}
      </div>
...
function ToolItem({ icon, title, desc, action, onClick, disabled }) {
  return (
    <div className={`tool-item ${disabled ? 'opacity-50' : ''}`}>
      <div className="tool-icon-wrapper" style={{ color: '#111827', marginTop: '2px' }}>{icon}</div>
      <div className="tool-info"><span className="tool-name">{title}</span><span className="tool-desc">{desc}</span></div>
      <button onClick={disabled ? null : onClick} disabled={disabled} className="tool-action-btn" style={{ padding: '6px 12px', fontSize: '12px', fontWeight: '500', color: disabled ? '#94a3b8' : '#111827', background: disabled ? '#f9fafb' : '#f3f4f6', border: '1px solid #e5e7eb', borderRadius: '6px', cursor: disabled ? 'not-allowed' : 'pointer', transition: 'all 0.2s' }}>
