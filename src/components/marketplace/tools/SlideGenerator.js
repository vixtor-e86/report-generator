...
export default function SlideGenerator({ 
  ...
  // Restore saved presentations from local storage on mount
  useEffect(() => {
    try {
      const saved = localStorage.getItem('w3_saved_slides_history');
      if (saved) {
        setSavedPresentations(JSON.parse(saved));
      }
    } catch (e) {
      console.error('Failed to load saved slides history:', e);
    }
  }, []);
  ...
  const handleProcess = async (isIterative = false, skipPaymentCheck = false) => {
    ...
    try {
      ...
      // Auto-save to presentation history tab
      const newEntry = {
        id: Date.now().toString(),
        title: data.data.title || 'Untitled Presentation',
      ...
      const updatedHistory = [newEntry, ...savedPresentations.filter(p => p.title !== newEntry.title)];
      setSavedPresentations(updatedHistory);
      try {
        localStorage.setItem('w3_saved_slides_history', JSON.stringify(updatedHistory));
      } catch (e) {
        console.error('Failed to save presentation to history:', e);
      }

      toast.success('Presentation Generated & Saved to History!');
      setHasPaid(false);
    } catch (err) {
  ...
  const handleDeleteSaved = (id, e) => {
    ...
    try {
      localStorage.setItem('w3_saved_slides_history', JSON.stringify(updated));
      toast.success('Presentation deleted from history');
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const handleLoadSaved = (item) => {
    ...
    setCurrentIndex(0);
    setActiveTab('create');
    toast.info(`Loaded presentation: ${item.title}`);
  };
      ...
      <div className="space-y-6 animate-in fade-in duration-500">
        <div className="flex items-center justify-between bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
          ...
          <button 
            ...
            className="px-4 py-2 bg-slate-900 text-white rounded-xl font-bold text-xs hover:bg-black transition-all flex items-center gap-1.5"
          >
            <Plus className="w-3.5 h-3.5" /> Create Another Presentation
          </button>
        </div>

        <div className="space-y-8 md:space-y-12 slide-in-from-bottom-4 duration-700">
          <div className="flex flex-col lg:flex-row gap-8 md:gap-12">
              <div className="flex-1 min-w-0">
                  <div className="bg-white p-6 md:p-8 rounded-[32px] md:rounded-[48px] shadow-2xl border border-slate-100">
                      <div className="flex justify-between items-center mb-6 md:mb-8 text-zinc-900">
              ...
              <div className="lg:w-96 space-y-6 md:space-y-8">
                  ...
                  <div className="bg-white p-6 md:p-8 rounded-[32px] md:rounded-[40px] border border-slate-100 shadow-xl">
                      ...
                      <div className="space-y-4">
                          ...
                          <Button onClick={handleDownloadPPTX} className="w-full bg-blue-600 hover:bg-blue-700 text-white rounded-xl md:rounded-2xl py-6 md:py-8 font-black uppercase text-[10px] md:text-[11px] tracking-widest shadow-lg flex items-center justify-center gap-2 md:gap-3"><Download className="w-4 h-4 md:w-5 md:h-5" /> Download PPTX</Button>
                          <button onClick={() => { setSlides([]); setInput(''); }} className="w-full text-[9px] md:text-[10px] font-black text-slate-600 uppercase tracking-widest hover:text-red-500 transition-colors">Create Another Presentation</button>
                      </div>
                  </div>
              </div>
          </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-white p-3 rounded-3xl border border-slate-200 shadow-sm">
        <div className="flex items-center gap-2">
          <button
            ...
            className={`flex items-center gap-2 px-5 py-3 rounded-2xl font-black text-xs uppercase tracking-wider transition-all ${
              activeTab === 'create' ? 'bg-zinc-900 text-white shadow-md' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'
            }`}
          >
            <Monitor className="w-4 h-4 text-blue-500" /> Create Presentation
          </button>

          <button
            onClick={() => setActiveTab('saved')}
            className={`flex items-center gap-2 px-5 py-3 rounded-2xl font-black text-xs uppercase tracking-wider transition-all ${
              activeTab === 'saved' ? 'bg-zinc-900 text-white shadow-md' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'
            }`}
          >
            <BookOpen className="w-4 h-4 text-emerald-500" /> Saved Presentations
            {savedPresentations.length > 0 && (
              <span className="bg-blue-600 text-white text-[10px] font-black px-2 py-0.5 rounded-full ml-1">
        ...
        {activeTab === 'saved' && (
          <Button
            onClick={() => setActiveTab('create')}
            size="sm"
            className="bg-black hover:bg-zinc-800 text-white rounded-2xl font-black text-xs uppercase tracking-widest px-5"
          >
            <Plus className="w-4 h-4 mr-1.5" /> New Presentation
          </Button>
        )}
      </div>

      {/* VIEW: Saved Presentations History */}
      {activeTab === 'saved' ? (
        <div className="bg-white border border-[#e5e7eb] rounded-[32px] md:rounded-[40px] p-6 md:p-10 shadow-sm space-y-6">
          <div className="flex items-center justify-between border-b border-slate-100 pb-6">
            <div>
              <h2 className="text-lg md:text-xl font-black text-slate-900 uppercase tracking-tight">Your Generated Presentations</h2>
              <p className="text-xs text-slate-500 font-medium">Access your previously created slide decks, preview them, or export as PPTX.</p>
            </div>
            <Badge variant="outline" className="font-bold text-xs uppercase tracking-wider text-slate-600 px-3 py-1">
              {savedPresentations.length} Saved Decks
            </Badge>
          </div>
          ...
          {savedPresentations.length === 0 ? (
            <div className="py-16 text-center space-y-4">
              ...
              <div>
                <h3 className="text-base font-black text-slate-900 uppercase">No Saved Presentations Yet</h3>
                <p className="text-xs text-slate-500 font-medium max-w-sm mx-auto mt-1">
                  Once you generate a slide deck, it will automatically be stored here so you can revisit it anytime.
                </p>
              </div>
              <Button onClick={() => setActiveTab('create')} className="bg-black text-white rounded-2xl font-black text-xs uppercase tracking-widest px-6 py-4">
                Build First Presentation →
              </Button>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {savedPresentations.map((item) => (
                <div 
                  ...
                  className="bg-slate-50 hover:bg-white border-2 border-slate-100 hover:border-zinc-900 rounded-3xl p-6 shadow-sm hover:shadow-xl transition-all cursor-pointer group relative flex flex-col justify-between"
                >
                  <div>
                    <div className="flex items-start justify-between gap-3 mb-3">
                      <span className="bg-blue-100 text-blue-700 font-black text-[10px] uppercase tracking-widest px-3 py-1 rounded-full">
                    ...
                    <h3 className="text-base font-black text-slate-900 uppercase tracking-tight line-clamp-2 mb-1 group-hover:text-indigo-600 transition-colors">
                      {item.title}
                    </h3>
                    {item.subtitle && (
                      <p className="text-xs text-slate-500 font-medium line-clamp-1 mb-3">{item.subtitle}</p>
                    )}
                  </div>

                  <div className="pt-4 border-t border-slate-200/60 flex items-center justify-between text-[10px] font-bold text-slate-400 uppercase tracking-wider">
        ...
        /* VIEW: Presentation Generator Form */
